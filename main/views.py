...
from .serializers import (
    ProductSerializer, ShoppingcartSerializers,
    CategorySerializer, QuerySerializer,
    EmailSerializer, ProductDocumentSerializer,
    SectionSerializer, CommentSerializer,
    LikeSerializer, OrderSerializer
)
from django.contrib.auth.views import get_user_model

from main.models import Products, Shoppingcart, Order, UserBalance, ArchiveShoppingcart, Sections, Category, \
    Subscription, Comment, Like
...
class AddShoppingcartAPIView(GenericAPIView):
    permission_classes = (IsAuthenticated,)
...
class CheckoutShoppingcart(APIView):
    ...
    def delete(self, request, product_id):
        ...
        if not Shoppingcart.objects.filter(Q(user_id=user_id) & Q(product_id=product_id)).exists():
            return Response(
                {'success': False, 'error': 'This user has no products with this id in their shopping cart'},
                status=404)
        else:
...
class LikeUserAPIView(GenericAPIView):
    ...
    def post(self, request, product_id):
        product = Products.objects.get(pk=product_id)
        if not Like.objects.filter(Q(user_id=request.user) & Q(product_id=product_id)).exists():
...
class CommentUserAPIView(GenericAPIView):
    ...
    def post(self, request, product_id):
        ...
        if not Comment.objects.filter(Q(user_id=request.user.id) & Q(product_id=product_id)).exists():
